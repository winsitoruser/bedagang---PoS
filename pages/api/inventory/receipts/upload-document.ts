...
const apiLogger = logger.child({ service: 'receipt-document-upload-api' });
...
export default async function handler(
  ...
  try {
    ...
    const receiptId = fields.receiptId?.[0];
    ...
    try {
      const receiptExists = await db.Receipt.findOne({
      ...
      apiLogger.warn('Error checking receipt exists:', error);
    ...
    for (const file of Array.isArray(uploadedFiles) ? uploadedFiles : [uploadedFiles]) {
      ...
      const documentData = {
        ...
        size: file.size,
        category: fields.category?.[0] || 'receipt',
        receiptId: receiptId,
        tenantId,
        createdBy: session?.user?.id,
        description: fields.description?.[0] || 'Dokumen Penerimaan',
    ...
    apiLogger.error('Error in receipt document upload API handler:', error);
