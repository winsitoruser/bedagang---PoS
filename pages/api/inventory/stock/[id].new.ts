 ...
 * Operasi dengan timeout protection dan fallback ke mock data
 */
...
export default withApiHandler(async function handler(
  ...
  try {
    ...
    const cacheKey = `${STOCK_ITEM_CACHE_KEY}:${tenantId}:${id}`;
    
    // Check cache first for optimized performance
    const cachedData = await cacheManager.getCachedData<{ item: StockItem | null; isMock: boolean }>(STOCK_ITEM_CACHE_KEY, {
      cacheKey
    ...
    try {
      ...
      const result = await Promise.race([getItemPromise, timeoutPromise]) as { item: StockItem | null; isMock: boolean };
      ...
      return success(res, {
        message: result.isMock
          ? 'Mock stock item details retrieved successfully (fallback mode)'
          : 'Stock item details retrieved successfully',
        data: result.item,
        meta: {
          isMock: result.isMock
        }
